...
This repository contains the data pipeline and analysis code for our master's thesis examining the effect of data centre entry in Virginia census tracts on wholesale   
electricity prices, as measured by PJM Locational Marginal Prices (LMPs). The empirical strategy combines propensity score matching (PSM) with a difference-in-differences (DiD) design.                                                               
A note on large data files: Due to file size limits, the full raw PJM LMP data (filtered to identified Virginia pnodes) is accessible via a Google drive folder: https://drive.google.com/drive/folders/19szrJjiv3aYtpBLJYDOeCehJksi4n2yY
